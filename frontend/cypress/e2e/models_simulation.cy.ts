describe('Models and Simulation Flow', () => {
  beforeEach(() => {
    // 1. Автентифікація
    cy.visit('/auth');
  ...
  const runSimulationAndVerifyResult = (modelName: string, steps: string, params: { [key: string]: string }, initial: string, expectedH3Title: string, visualizationModes: string[]) => {
    ...
    Object.keys(params).forEach(key => {
      cy.get(`#${key}`).clear().type(params[key]);
    });
  ...
  it('should display the Models page title and navigate to History', () => {
  ...
  it('should successfully run a Henon Map simulation and check 2D plots', () => {
    runSimulationAndVerifyResult(
      'Henon Map',
      '1000',
      { a: '1.4', b: '0.3' },
      '0.1, 0.3',
  ...
  it('should successfully run a Lorenz Attractor simulation and check 2D/3D plots', () => {
    runSimulationAndVerifyResult(
      'Lorenz Attractor',
      '1000',
  ...
  it('should successfully run a Thomas Attractor simulation and check 2D/3D plots', () => {
    runSimulationAndVerifyResult(
      'Thomas Attractor',
      '1000',
      { b: '0.18', dt: '0.01' },
  ...
  it('should display created simulations in history and allow viewing details', () => {
